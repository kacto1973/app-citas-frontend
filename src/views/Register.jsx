  ...
  const handleRegister = (e) => {
    ...
    const asyncFunc = async () => {
      const result = await registerClient(
        fullName, localStorage.getItem("cellphone")
      );
      if (result) {
        alert("Usuario registrado con éxito con el número de celular: " + localStorage.getItem("cellphone"));
        navigate("/");
      }
    };
    asyncFunc();
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden bg-[linear-gradient(194deg,#4C2DFF_0%,#DE9FFE_100%)] ">
      ...
      <div className="top-[34%] absolute flex flex-col left-1/2 -translate-x-1/2 w-[85%] rounded-3xl p-5 bg-g7 text-center">
                <h2 className=" text-black text-2xl font-black mb-6">Ingresa tu Nombre Completo</h2>
       ...
       id="outlined-basic" label="Nombre Completo" variant="outlined" />
